...
- The async-job convention (`?async`, `X-Async`, `X-Idempotency-Key`, `X-Job-Name`, `X-Job-Rationale`, `X-Provider` headers; `X-Job-Id`/`X-Result-Json` response headers) appears **nowhere** in /discover.
...
| Nova triggers RedCompute jobs (oneshot, TTS, image gen) | ✅ | ✅ mostly | Execute timeout docs wrong; sandbox/docker params undocumented; async headers (`X-Async`, `X-Idempotency-Key`) in no manifest; multi-clip music output unreachable |
...
| I13 | RedCompute: surface the async-job contract (`?async`, `X-Async`, `X-Idempotency-Key`, `X-Job-Id`/`X-Result-Json`) in the manifest — as registry params or a `conventions` block in /discover | `RedComputeServiceDescriptor.cs` / registry entries | The suite's most important invocation pattern is pure folklore | Small |
...
| I13 RedCompute async-job contract surfaced | ✅ | in `548a426`/`22c7355` — `?async` + X-Async/X-Idempotency-Key/X-Job-Name/X-Job-Rationale/X-Provider documented with header locations. (`/ai-session/generate` does not read `?async` — documented only where the handler supports it) |
