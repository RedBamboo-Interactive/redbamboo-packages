...
export function ToolOutputView({ content, isError }: Props) {
  ...
  return (
    <div className={isError ? "rounded-md bg-red-500/[0.06] px-3 py-2 -mx-3 -mb-1" : undefined}>
      {parsedJson ? (
        <JsonHighlight json={display} />
      ) : (
        <pre className={`text-xs font-mono whitespace-pre-wrap break-all ${isError ? "text-red-300/80" : ""}`}>
          {display}
        </pre>
      )}
      {truncated && (
