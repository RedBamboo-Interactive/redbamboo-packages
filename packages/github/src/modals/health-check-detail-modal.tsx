      ...
      <div className="bg-surface border border-border rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 min-h-0 overflow-auto p-4 space-y-4">
          ...
          {healthCheck.status === "failed" && healthCheck.error_message && (
            <div className="bg-red-500/10 text-red-400 rounded-md p-3 text-sm">{healthCheck.error_message}</div>
          )}

          {Object.keys(suiteResults).length > 0 && (
            <div className="space-y-2">
              <h3 className="text-xs font-medium text-text-muted uppercase tracking-wide">Suite Results</h3>
              {Object.entries(suiteResults).map(([key, r]) => {
                const suite = r as HealthCheckSuiteResult
                return (
                  <div
                    key={key}
                    className={`rounded-md p-3 text-sm ${suite.passed ? "bg-emerald-500/5" : "bg-red-500/5"}`}
                  >
                    <div className="flex items-center gap-2">
                      {suite.passed ? (
                        <i className="fa-solid fa-circle-check text-sm text-emerald-400" />
                      ...
                      <span className="text-xs text-text-muted ml-auto">
                        ...
                        {suite.duration_seconds > 0 && (
                          <span className="ml-1">({suite.duration_seconds.toFixed(1)}s)</span>
                        )}
                      </span>
                    </div>
                    {suite.error && <p className="text-xs text-red-400 mt-1 ml-6">{suite.error}</p>}
                  </div>
                )
              })}
            </div>
