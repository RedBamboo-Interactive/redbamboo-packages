    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-surface border border-border rounded-lg shadow-xl w-full max-w-lg mx-4">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          ...
          <button onClick={handleClose} className="text-text-muted hover:text-contrast text-lg leading-none">
            &times;
          </button>
        </div>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {LABEL_OPTIONS.map((label) => (
              <button key={label} onClick={() => toggleLabel(label)} disabled={submitting}>
                <Badge
          ...
          {result && !submitting && (
            <div className={`flex items-start gap-2 text-sm rounded-md p-3 ${
              ...
              {result.ok ? (
                <>
                  <i className="fa-solid fa-circle-check text-sm mt-0.5 flex-none" />
                  <div>
                    ...
                    {result.issue_url && (
                      ...
                      >
                        View on GitHub <i className="fa-solid fa-arrow-up-right-from-square text-[9px]" />
                      </a>
                    )}
                  </div>
                </>
                ...
                <>
                  <i className="fa-solid fa-circle-xmark text-sm mt-0.5 flex-none" />
                  <p>{result.error}</p>
                </>
              )}
            </div>
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-border">
          ...
          {!result?.ok && (
            ...
            >
              {submitting ? (
                <>
                  <i className="fa-solid fa-spinner fa-spin text-[11px] mr-1.5" />
                  Creating...
                </>
              ) : (
                "Create Issue"
